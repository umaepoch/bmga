    ...
    if len(items) > 1:
        if len(warehouse) > 1:
            stock_data_batch = frappe.db.sql(f"""
                select batch_id , `tabBatch`.stock_uom, item as item_code, expiry_date, sum(`tabStock Ledger Entry`.actual_qty) as actual_qty
                from `tabBatch`
            ...
            stock_data_batchless = frappe.db.sql(
                f"""select batch_no as batch_id, item_code, stock_uom, sum(actual_qty) as actual_qty from `tabStock Ledger Entry`
            ...
            stock_data_batchless = frappe.db.sql(
                f"""select batch_no as batch_id, item_code, stock_uom, sum(actual_qty) as acutal_qty from `tabStock Ledger Entry`
...
def sales_order_handle(sales_list, stock_data, expiry_date):
...
def item_list_container(so_name, company):
    ...
    pick_put_list = sales_order_handle(sales_list, handled_data, 120)
